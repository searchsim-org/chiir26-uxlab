                      ...
                      <div className="space-y-6">
                        ...
                        {step.type === 'questionnaire' && (
                          <div>
                            ...
                            <button
                              onClick={() => {
                                // Toggle: if questions already exist, show/hide; otherwise init with one empty question
                                ...
                                // Use a data attribute on the step to toggle visibility
                                updateStep(step.id, { questionnaireType: step.questionnaireType === 'custom-editing' ? 'custom' : 'custom-editing' });
                              }}
                              className="mt-3 text-sm text-blue-500 hover:text-blue-600 font-medium"
                            >
                              {step.questionnaireType === 'custom-editing' ? '- Hide Questions' : '+ Customize Questions'}
                            </button>

                            {/* Inline Question Editor */}
                            {step.questionnaireType === 'custom-editing' && (
                              <div className="mt-4 space-y-4 border-t border-border pt-4">
                                {(step.questions ?? []).map((q, qi) => (
                                  <div key={q.id} className="bg-secondary rounded-xl p-4 border border-border space-y-3">
                                    <div className="flex items-center justify-between">
                                      <span className="text-xs font-semibold text-muted-foreground">Question {qi + 1}</span>
                                      <button
                                        onClick={() => {
                                          const updated = (step.questions ?? []).filter((_, i) => i !== qi);
                                          updateStep(step.id, { questions: updated });
                                        }}
                                    ...
                                    <input
                                      ...
                                      className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                    <div className="flex items-center space-x-3">
                                      <select
                                        value={q.type}
                                        onChange={(e) => {
                                          const updated = [...(step.questions ?? [])];
                                          updated[qi] = { ...updated[qi], type: e.target.value as QuestionnaireQuestion['type'] };
                                          updateStep(step.id, { questions: updated });
                                        }}
                                        className="bg-background border border-border rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500"
                                      >
                                      ...
                                      <label className="flex items-center space-x-1.5 text-xs text-muted-foreground">
                                        <input
                                          ...
                                          onChange={(e) => {
                                            const updated = [...(step.questions ?? [])];
                                            updated[qi] = { ...updated[qi], required: e.target.checked };
                                            updateStep(step.id, { questions: updated });
                                          }}
                                    ...
                                    {(q.type === 'choice' || q.type === 'likert') && (
                                      <div>
                                        ...
                                        <input
                                          type="text"
                                          value={(q.options ?? (q.type === 'likert' ? ['Strongly Disagree', 'Disagree', 'Neutral', 'Agree', 'Strongly Agree'] : [])).join(', ')}
                                          onChange={(e) => {
                                            const updated = [...(step.questions ?? [])];
                                            updated[qi] = { ...updated[qi], options: e.target.value.split(',').map(s => s.trim()).filter(Boolean) };
                                            updateStep(step.id, { questions: updated });
                                          }}
                                          placeholder="Option 1, Option 2, Option 3"
                                          className="w-full bg-background border border-border rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500"
                                        />
                                      </div>
                                    )}
                                    {q.type === 'scale' && (
                                      <div className="flex items-center space-x-3">
                                        <div>
                                          ...
                                          <input
                                            ...
                                            onChange={(e) => {
                                              const updated = [...(step.questions ?? [])];
                                              updated[qi] = { ...updated[qi], scaleMin: parseInt(e.target.value) || 1 };
                                              updateStep(step.id, { questions: updated });
                                            }}
                                            className="w-16 bg-background border border-border rounded-lg px-2 py-1 text-xs outline-none"
                                          />
                                        </div>
                                        <div>
                                          ...
                                          <input
                                            ...
                                            onChange={(e) => {
                                              const updated = [...(step.questions ?? [])];
                                              updated[qi] = { ...updated[qi], scaleMax: parseInt(e.target.value) || 7 };
                                              updateStep(step.id, { questions: updated });
                                            }}
                                            className="w-16 bg-background border border-border rounded-lg px-2 py-1 text-xs outline-none"
                                          />
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    updateStep(step.id, { questions: [...(step.questions ?? []), newQ] });
                                  }}
                                  className="w-full border-2 border-dashed border-border hover:border-blue-500 rounded-xl px-4 py-2.5 text-xs text-muted-foreground hover:text-foreground transition-all flex items-center justify-center space-x-2"
                                >
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                  </svg>
